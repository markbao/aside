...
function addBookmark(data) {
	...
	if (typeof data.tags === 'string')
		newMark.tags = (data.tags) ? data.tags.split(', ') : [];
	newMark.time = new Date().toISOString().split('.')[0] + 'Z';
	newMark.ident = null;
	newMark.hits = 0;
	allBookmarks.unshift(newMark);
}
...
function handleButtonPush(e) {
	switch (e.target) {
		...
		case reloadButton: case reloadButtonImg: {
			e.stopPropagation();
			allBookmarks = null;
			popover ? gw.getAllBookmarks(refresh) : safari.self.tab.dispatchMessage('reloadBookmarks');
			break;
		}
...
function handleInputKeyDown(e) {
	...
	switch (e.which) {
		...
		case 33: {  //pageup
			...
			if (selectedBmItem) {
				for (var i = 0; i < bookmarkList.childNodes.length; i++) {
					if (bookmarkList.childNodes[i] === selectedBmItem) {
						var sbIndex = i;
						break;
					}
				}
				var vil = bookmarkList.offsetHeight / selectedBmItem.offsetHeight;
		...
		case 38: {  //up
			e.preventDefault();
			if (selectedBmItem) {
				selectListItem(selectedBmItem.previousSibling || bookmarkList.lastChild);
			} break;
		}
		case 9 : {  //tab
		}
		case 40: {  //down
			e.preventDefault();
			if (selectedBmItem) {
				selectListItem(selectedBmItem.nextSibling || bookmarkList.firstChild);
			} break;
		}
	}
}
function handleInputKeyUp(e) {
	...
	var specialKeys = [8, 127, 186, 191];
	if (specialKeys.indexOf(e.which) > -1 ) {
		console.log('inputField.value: "' + inputField.value + '"');
		console.log('bookmarkList.listType:', bookmarkList.listType);
		if (inputField.value == '') {
...
function selectListItem(li, index) {
	...
	if (selectedBmItem) {
		selectedBmItem.className = selectedBmItem.className.replace(' selected', '');
		selectedBmItem.className = selectedBmItem.className.replace(' showurl', '');
	}
	selectedBmItem = li || bookmarkList.childNodes[index];
	if (selectedBmItem) {
		selectedBmItem.className += ' selected';
		selectedBmItem.scrollIntoViewIfNeeded(false);
		window.urlRevealTimer = setTimeout(function () {
			selectedBmItem.className += ' showurl';
			delete window.urlRevealTimer;
		}, 1000);
	}
}
...
function updateBmList(bookmarks, listType) {
	...
	selectedBmItem = bookmarkList.firstChild;
	if (listType === 'recent')
	...
	selectListItem(selectedBmItem);
	var itemHeight = selectedBmItem.offsetHeight;
